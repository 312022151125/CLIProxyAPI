...
func ComputeOpenAICompatModelsHash(models []config.OpenAICompatibilityModel) string {
	keys := modelRoutingKeys(func(out func(key string)) {
		for _, model := range models {
			...
			out(strings.ToLower(name) + "|" + strings.ToLower(alias) + "|" + strings.TrimSpace(model.DisplayName) + "|" + fmt.Sprintf("image=%t", model.ImageEnabled()) + "|" + fmt.Sprintf("video=%t", model.VideoEnabled()) + "|" + fmt.Sprintf("force-mapping=%t", model.GetForceMapping()) + "|" + fmt.Sprintf("is-compat=%t", model.IsCompat) + "|" + fmt.Sprintf("use-max-completion-tokens=%t", model.UseMaxCompletionTokens) + "|input=" + strings.Join(normalizeModalities(model.InputModalities), ",") + "|output=" + strings.Join(normalizeModalities(model.OutputModalities), ",") + thinkingHashSuffix(model.Thinking))
		}
	})
...
func ComputeVertexCompatModelsHash(models []config.VertexCompatModel) string {
	keys := modelRoutingKeys(func(out func(key string)) {
		for _, model := range models {
			...
			out(strings.ToLower(name) + "|" + strings.ToLower(alias) + "|" + strings.TrimSpace(model.DisplayName) + "|" + fmt.Sprintf("force-mapping=%t", model.GetForceMapping()) + thinkingHashSuffix(model.Thinking))
		}
	})
	return hashJoined(keys)
}
...
func ComputeClaudeModelsHash(models []config.ClaudeModel) string {
	keys := modelRoutingKeys(func(out func(key string)) {
		for _, model := range models {
			...
			out(strings.ToLower(name) + "|" + strings.ToLower(alias) + "|" + strings.TrimSpace(model.DisplayName) + "|" + fmt.Sprintf("force-mapping=%t", model.GetForceMapping()) + "|" + fmt.Sprintf("is-compat=%t", model.IsCompat) + thinkingHashSuffix(model.Thinking))
		}
	})
	return hashJoined(keys)
}
